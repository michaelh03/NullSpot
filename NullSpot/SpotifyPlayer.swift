...
private nonisolated func registerAudioDataCallback() {
    nullspot_register_audio_data_callback { samples, count in
...
private nonisolated func registerAudioControlCallback() {
    nullspot_register_audio_control_callback { event in
        switch event {
...
private nonisolated func registerQueueCallback() {
    nullspot_register_queue_callback { jsonPtr in
...
private nonisolated func registerPlaybackStateCallback() {
    nullspot_register_playback_state_callback { jsonPtr in
...
private nonisolated func registerStateUpdateCallback() {
    nullspot_register_state_update_callback {
...
private nonisolated func registerVolumeCallback() {
    nullspot_register_volume_callback { volume in
...
private nonisolated func registerLoadingCallback() {
    nullspot_register_loading_callback { jsonPtr in
...
private nonisolated func registerQueueChangedCallback() {
    nullspot_register_queue_changed_callback { jsonPtr in
...
private nonisolated func registerConnectionStateCallback() {
    nullspot_register_connection_state_callback { jsonPtr in
...
private nonisolated func handleConnectionStateCallback(_ jsonPtr: UnsafePointer<CChar>?) {
    ...
    do {
        ...
        let state = LibrespotConnectionState(
            ...
            deviceName: json["device_name"] as? String ?? "NullSpot",
...
private nonisolated func registerSetQueueCallback() {
    nullspot_register_set_queue_callback { jsonPtr in
...
private nonisolated func registerSessionDisconnectedCallback() {
    nullspot_register_session_disconnected_callback {
...
private nonisolated func registerSessionConnectedCallback() {
    nullspot_register_session_connected_callback {
...
private nonisolated func registerTokenRequestCallback() {
    nullspot_register_token_request_callback {
...
private nonisolated func registerSessionClientChangedCallback() {
    nullspot_register_session_client_changed_callback { jsonPtr in
...
enum SpotifyPlayer {
    ...
    static var isSessionConnected: Bool {
        nullspot_is_session_connected() == 1
    }
    ...
    static func forceReconnect() -> Bool {
        let result = nullspot_force_reconnect()
        return result == 0
    }
    ...
    /// Returns a fresh keymaster token minted via login5 on the current librespot Session.
    /// This token bypasses Dev Mode restrictions (same path spotify-player uses).
    /// Returns nil if no Session is connected, or an error string prefixed "ERR: " on failure.
    /// Blocks the calling thread on a Mercury round-trip (up to 5s) — call from a
    /// detached task, not the main actor.
    nonisolated static func getLogin5Token() -> String? {
        guard let ptr = nullspot_get_login5_token() else { return nil }
        defer { nullspot_free_string(ptr) }
        return String(cString: ptr)
    }

    /// Mints a login5 keymaster token from a transient librespot Session created with
    /// the supplied OAuth access token (typically a desktop client_id PKCE token).
    /// Does NOT touch the global Session used for Spotify Connect.
    /// Returns an error string prefixed "ERR: " on failure.
    /// Blocks the calling thread up to ~25s (AP handshake + login5) — call from a
    /// detached task.
    nonisolated static func mintLogin5TokenFromOAuth(_ oauthToken: String) -> String? {
        oauthToken.withCString { cstr in
            guard let ptr = nullspot_mint_login5_token_from_oauth(cstr) else { return nil }
            defer { nullspot_free_string(ptr) }
            return String(cString: ptr)
        }
    }
    ...
    static func getConnectionState() -> LibrespotConnectionState? {
        let ptr = nullspot_get_connection_state()
    ...
    private nonisolated static func syncSettingsFromUserDefaults() {
        ...
        nullspot_set_bitrate(UInt8(min(max(bitrateRawValue, 0), 2)))
        nullspot_set_gapless(gaplessEnabled)
        nullspot_set_initial_volume(volumeU16)
    }
    ...
    static func play(uriOrUrl: String, trackIndex: Int = -1) async throws {
        debugLog("QTRACE", "SpotifyPlayer.play -> nullspot_play_uri uri=\(uriOrUrl) trackIndex=\(trackIndex)")
        let result = await Task.detached {
            uriOrUrl.withCString { ptr in
                nullspot_play_uri(ptr, Int32(trackIndex))
            }
        }.value

        debugLog("QTRACE", "SpotifyPlayer.play nullspot_play_uri result=\(result)")
        guard result == 0 else {
    ...
    static func transferPlayback(to deviceId: String) {
        Task.detached(priority: .userInitiated) {
            _ = deviceId.withCString { ptr in
                nullspot_transfer_playback(ptr)
            }
        }
    }
