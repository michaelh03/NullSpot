...
final class SpotifySession {
    ...
    /// Force the cached token to be treated as expired so the next
    /// `validAccessToken()` call performs a refresh. Used by the API layer
    /// when a request gets a 401 despite the local expiry check thinking
    /// the token was still valid (server-side revocation, clock skew, etc).
    func invalidateToken() {
        expiresIn = 0
        // Clear the failure-backoff so an explicit invalidation isn't suppressed
