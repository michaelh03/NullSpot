...
private final class DriftCorrectionTimer {
    ...
    func start() {
        ...
        thread.name = "com.nullspot.drift-correction"
...
final class PlaybackViewModel {
    ...
    /// True from when play()/playTracks() is invoked until Spirc confirms the
    /// track has actually started playing. While true, pause()/resume() no-op
    /// so a user click during the CDN load window can't trigger Rust's
    /// `Resume fallback: loading context` cascade (which fires redundant Loads
    /// and turns a ~7s wait into ~14s). Cleared by the first Spirc state
    /// update reporting playback, or by a 10s safety timeout.
    ...
    var volume: Double = 0.5 {
        didSet {
            ...
            // Only persist when NullSpot is the active device; don't overwrite local volume
    ...
    /// Volume of the active remote Spotify Connect device (nil when NullSpot is active).
    ...
    /// In-flight init task — used to dedupe concurrent initializeIfNeeded() callers
    /// (e.g. lifecycle .task + a user click on play happening before the lifecycle init finishes).
    ...
    /// Session reference used for SpotifyAPI calls and reinitialization after session disconnect
    private weak var session: SpotifySession?
    ...
    private init() {
        ...
        initialInfo[MPMediaItemPropertyTitle] = "NullSpot"
    ...
    /// Sets the session reference for SpotifyAPI calls and reinitialization after session disconnect.
    func setSession(_ session: SpotifySession) {
    ...
    func forceReinitialize(session _: SpotifySession) async {
        ...
        do {
            ...
            // Only mark initialized once Spirc is actually ready — callers like play()
            // gate on isInitialized and would otherwise fire into a not-yet-ready Spirc.
            isInitialized = SpotifyPlayer.isSpircReady
        } catch {
    ...
    func initializeIfNeeded(session _: SpotifySession) async {
        ...
        let task = Task { [weak self] in
            ...
            do {
                ...
                // Only mark initialized once Spirc is actually ready. Otherwise the next
                // play() call would skip init and fire SpotifyPlayer.play() into a Spirc
                // that hasn't connected yet, silently dropping the user's first play.
                isInitialized = SpotifyPlayer.isSpircReady
            } catch {
                errorMessage = error.localizedDescription
            }
    ...
    func play(uriOrUrl: String, trackIndex: Int = -1, session: SpotifySession) async {
    ...
    func playTrack(trackId: String, session: SpotifySession) async {
    ...
    func addToQueue(uri: String, session: SpotifySession) async {
        // Initialize if needed
        if !isInitialized {
            await initializeIfNeeded(session: session)
        }
        ...
        if SpotifyPlayer.isActiveDevice, SpotifyPlayer.isSessionConnected {
            // Local Spirc path — queue update will come via Mercury callback
            SpotifyPlayer.addToQueue(uri: uri)
        } else {
            // Remote device — POST /me/player/queue
            do {
                try await SpotifyAPI.addToQueue(session: session, uri: uri)
            } catch {
                errorMessage = error.localizedDescription
                debugLog("PlaybackViewModel", "addToQueue Web API failed: \(error)")
            }
        }
    }
    ...
    /// Open the awaiting-playback gate. Called when we issue a Spirc play()/playTracks();
    /// closed by the first Spirc state update with isPlaying=true, or after 10s as a
    /// failsafe so a failed load doesn't permanently block the transport buttons.
    private func beginAwaitingPlayback() {
        isAwaitingPlayback = true
        awaitingPlaybackTimeoutTask?.cancel()
        awaitingPlaybackTimeoutTask = Task { [weak self] in
            try? await Task.sleep(for: .seconds(10))
            guard !Task.isCancelled else { return }
            await MainActor.run {
                guard let self, self.isAwaitingPlayback else { return }
                self.endAwaitingPlayback()
            }
        }
    }

    private func endAwaitingPlayback() {
        isAwaitingPlayback = false
        awaitingPlaybackTimeoutTask?.cancel()
        awaitingPlaybackTimeoutTask = nil
    }

    func togglePlayPause(trackId: String, session: SpotifySession) async {
        if isPlaying, currentTrackUri == trackId {
            ...
            // Play new track
            await playTrack(trackId: trackId, session: session)
        }
    }
    ...
    func next() {
        if SpotifyPlayer.isActiveDevice {
            ...
            Task {
                ...
                do {
                    try await SpotifyAPI.skipToNext(session: session)
                } catch {
                    errorMessage = error.localizedDescription
                }
            }
    ...
    func previous() {
        if SpotifyPlayer.isActiveDevice {
            ...
            Task {
                guard let session else { return }
                do {
                    try await SpotifyAPI.skipToPrevious(session: session)
                } catch {
                    errorMessage = error.localizedDescription
                }
            }
    ...
    func pause() {
        ...
        if SpotifyPlayer.isActiveDevice {
            ...
            Task {
                guard let session else { return }
                do {
                    try await SpotifyAPI.pausePlayback(session: session)
                } catch {
                    errorMessage = error.localizedDescription
                }
            }
    ...
    func resume() {
        ...
        if SpotifyPlayer.isActiveDevice {
            ...
            Task {
                guard let session else { return }
                do {
                    try await SpotifyAPI.resumePlayback(session: session)
                } catch {
                    errorMessage = error.localizedDescription
                }
            }
    ...
    func toggleShuffle() {
        ...
        if SpotifyPlayer.isActiveDevice {
            ...
            Task {
                guard let session else { return }
                do {
                    try await SpotifyAPI.setShuffle(session: session, enabled: targetShuffle)
                } catch {
                    errorMessage = error.localizedDescription
                }
            }
    ...
    func toggleRepeat() {
        let targetEnabled = !isRepeatEnabled
        let mode: SpotifyAPI.RepeatMode = targetEnabled ? .context : .off
        isRepeatEnabled = targetEnabled
        Task {
            guard let session else { return }
            do {
                try await SpotifyAPI.setRepeat(session: session, mode: mode)
            } catch {
                isRepeatEnabled = !targetEnabled
                errorMessage = error.localizedDescription
            }
    ...
    private func setupVolumeSubscription() {
        volumeSubscription = SpotifyPlayer.volumeChanged
            ...
            .sink { [weak self] volumeU16 in
                ...
                // Only persist when NullSpot is the active device
                if remoteVolume == nil {
                    saveVolume()
                }
            }
    ...
    private func setupLoadingSubscription() {
        loadingSubscription = SpotifyPlayer.loading
            ...
            .sink { [weak self] notification in
                ...
                // clamping: protects against bogus values that would overflow UInt32.
                if notification.positionMs > 0 {
                    let posMs = UInt32(clamping: notification.positionMs)
    ...
    private func performSeek(to positionMs: UInt32) {
        ...
        if SpotifyPlayer.isActiveDevice {
            ...
            Task {
                guard let session else { return }
                do {
                    try await SpotifyAPI.seekToPosition(session: session, positionMs: Int(positionMs))
                } catch {
                    errorMessage = error.localizedDescription
                }
            }
    ...
    private func handlePlaybackStateUpdate(_ state: PlaybackState?) {
        ...
        // First confirmed playing event after a play()/playTracks() — release the
        // pause/resume gate so the user can control transport again.
        if newIsPlaying, isAwaitingPlayback {
            endAwaitingPlayback()
        }
        ...
        if state.durationMs > 0 {
            trackDurationMs = UInt32(clamping: state.durationMs)
        }

        isShuffleEnabled = state.shuffle
        isRepeatEnabled = state.repeatContext || state.repeatTrack
        ...
        if state.positionMs >= 0 {
            let posMs = UInt32(clamping: state.positionMs)
    ...
    func applyWebAPIPlaybackState(
        ...
        // Update duration
        if durationMs > 0 {
            trackDurationMs = UInt32(clamping: durationMs)
        }
        ...
        if progressMs >= 0 {
            let posMs = UInt32(clamping: progressMs)
            let now = CACurrentMediaTime()
    ...
    func toggleCurrentTrackFavorite(session: SpotifySession) async {
        ...
        do {
            if wasFavorite {
                try await SpotifyAPI.removeSavedTrack(session: session, trackId: trackId)
            } else {
                try await SpotifyAPI.saveTrack(session: session, trackId: trackId)
            }
        } catch {
            // Rollback
            if wasFavorite {
    ...
    /// Call when NullSpot becomes the active device.
            ...
            .sink { [weak self] newVolume in
                ...
                if SpotifyPlayer.isActiveDevice {
                    ...
                    Task { [weak self] in
                        ...
                        try? await SpotifyAPI.setVolume(session: session, percent: percent)
                    }
                }
            }
    }
