...
final class TopItemsService {
    ...
    func loadTopArtists(session: SpotifySession, timeRange: TopItemsTimeRange = .mediumTerm) async {
        guard !store.topArtistsPagination.isLoaded, !store.topArtistsPagination.isLoading else { return }
        await fetchTopArtistsPage(session: session, timeRange: timeRange, limit: 15, isRefresh: true)
    }

    /// Force refresh top artists (resets and loads first page)
    func refreshTopArtists(session: SpotifySession, timeRange: TopItemsTimeRange = .mediumTerm) async {
    ...
    func loadMoreTopArtists(session: SpotifySession, timeRange: TopItemsTimeRange = .mediumTerm) async {
        guard store.topArtistsPagination.hasMore, !store.topArtistsPagination.isLoading else { return }
        await fetchTopArtistsPage(session: session, timeRange: timeRange, limit: 50, isRefresh: false)
    }

    /// Fetch a single page of top artists
    private func fetchTopArtistsPage(session: SpotifySession, timeRange: TopItemsTimeRange, limit: Int, isRefresh: Bool) async {
        await fetchPage(
    ...
    func loadTopTracks(session: SpotifySession, timeRange: TopItemsTimeRange = .mediumTerm) async {
        guard !store.topTrackAlbumsPagination.isLoaded, !store.topTrackAlbumsPagination.isLoading else { return }
        await fetchTopTracksPage(session: session, timeRange: timeRange, limit: 15, isRefresh: true)
    }

    /// Force refresh top tracks and extract deduplicated albums
    func refreshTopTracks(session: SpotifySession, timeRange: TopItemsTimeRange = .mediumTerm) async {
    ...
    func loadMoreTopTracks(session: SpotifySession, timeRange: TopItemsTimeRange = .mediumTerm) async {
        guard store.topTrackAlbumsPagination.hasMore, !store.topTrackAlbumsPagination.isLoading else { return }
        await fetchTopTracksPage(session: session, timeRange: timeRange, limit: 50, isRefresh: false)
    }

    /// Fetch a single page of top tracks and extract deduplicated albums
    private func fetchTopTracksPage(session: SpotifySession, timeRange: TopItemsTimeRange, limit: Int, isRefresh: Bool) async {
        await fetchPage(
