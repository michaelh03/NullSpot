...
final class AlbumService {
    ...
    func loadUserAlbums(session: SpotifySession, forceRefresh: Bool = false) async throws {
    ...
    func loadMoreAlbums(session: SpotifySession) async throws {
        guard store.albumsPagination.hasMore, userAlbumsTask == nil else {
            return
        }
        try await loadUserAlbums(session: session)
    }

    // MARK: - Album Details

    /// Fetch album details and tracks
    func fetchAlbumDetails(albumId: String, session: SpotifySession) async throws -> Album {
    ...
    func getAlbumTracks(albumId: String, session: SpotifySession) async throws -> [Track] {
        ...
        let album = try await fetchAlbumDetails(albumId: albumId, session: session)
        return album.trackIds.compactMap { store.tracks[$0] }
    }

    /// Get album from store or fetch if needed
    func getAlbum(albumId: String, session: SpotifySession) async throws -> Album {
        if let album = store.albums[albumId] {
            return album
        }
        return try await fetchAlbumDetails(albumId: albumId, session: session)
    }

    // MARK: - Library Management

    /// Save an album to the user's library
    func saveAlbumToLibrary(albumId: String, session: SpotifySession) async throws {
    ...
    func removeAlbumFromLibrary(albumId: String, session: SpotifySession) async throws {
