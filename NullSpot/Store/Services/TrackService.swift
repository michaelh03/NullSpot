...
final class TrackService {
    ...
    func loadFavorites(session: SpotifySession, forceRefresh: Bool = false) async throws {
    ...
    func loadMoreFavorites(session: SpotifySession) async throws {
        guard store.favoritesPagination.hasMore, !store.favoritesPagination.isLoading else {
            return
        }
        try await loadFavorites(session: session)
    }

    // MARK: - Favorite Toggling (Optimistic)

    /// Toggle favorite status for a track (optimistic update)
    func toggleFavorite(trackId: String, session: SpotifySession) async throws {
    ...
    func checkFavoriteStatus(trackId: String, session: SpotifySession) async throws {
    ...
    func checkFavoriteStatuses(trackIds: [String], session: SpotifySession) async throws {
    ...
    func ensureFavoriteStatuses(trackIds: [String], session: SpotifySession) async {
        ...
        for batch in batches(of: unresolved, size: 50) {
            try? await checkFavoriteStatuses(trackIds: batch, session: session)
        }
    }

    /// Refresh favorite status for the given tracks even if we have stale cached data.
    func refreshFavoriteStatuses(trackIds: [String], session: SpotifySession) async {
    ...
    func fetchTrack(trackId: String, session: SpotifySession) async throws -> Track {
