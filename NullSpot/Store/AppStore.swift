...
final class AppStore {
    ...
    /// All shows (podcasts) indexed by ID
    private(set) var shows: [String: Show] = [:]
    ...
    /// Per-type pagination for the minimal search (one filter at a time).
    var searchTracksPagination = PaginationState()
    var searchAlbumsPagination = PaginationState()
    var searchArtistsPagination = PaginationState()
    var searchPlaylistsPagination = PaginationState()
    var searchShowsPagination = PaginationState()
    ...
    /// Upsert multiple shows
    func upsertShows(_ newShows: [Show]) {
    ...
    func clearSearch() {
        ...
        searchTracksPagination.reset()
        searchAlbumsPagination.reset()
        searchArtistsPagination.reset()
        searchPlaylistsPagination.reset()
        searchShowsPagination.reset()
    }

    /// Append additional tracks to the current search results (for infinite scroll).
    func appendSearchTracks(_ newTracks: [Track]) {
        guard let current = searchResults else { return }
    ...
    /// Append additional albums to the current search results.
    func appendSearchAlbums(_ newAlbums: [Album]) {
        guard let current = searchResults else { return }
        searchResults = SearchResults(
            albums: current.albums + newAlbums,
            artists: current.artists,
            playlists: current.playlists,
    ...
    /// Append additional artists to the current search results.
    func appendSearchArtists(_ newArtists: [Artist]) {
        guard let current = searchResults else { return }
        searchResults = SearchResults(
            albums: current.albums,
    ...
    /// Append additional playlists to the current search results.
    func appendSearchPlaylists(_ newPlaylists: [Playlist]) {
        guard let current = searchResults else { return }
        searchResults = SearchResults(
            albums: current.albums,
            artists: current.artists,
    ...
    /// Append additional shows to the current search results.
    func appendSearchShows(_ newShows: [Show]) {
        guard let current = searchResults else { return }
        searchResults = SearchResults(
            albums: current.albums,
            artists: current.artists,
