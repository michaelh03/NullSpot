...
final nonisolated class AudioRenderer: @unchecked Sendable {
    ...
    private let renderQueue = DispatchQueue(label: "com.nullspot.audio-renderer", qos: .userInteractive)
    ...
    func writeAudioData(_ samples: UnsafePointer<Float>, count: Int) {
        VisualizerTap.shared.submit(interleavedStereo: samples, sampleCount: count)
    ...
    /// (e.g. nullspot_disconnect expects flush to complete before proceeding).
    func start() {
