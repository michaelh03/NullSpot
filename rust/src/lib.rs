...
static TOKEN_REQUEST_CALLBACK: Lazy<Mutex<Option<extern "C" fn()>>> =
    Lazy::new(|| Mutex::new(None));
// Channel for receiving token from Swift (set via nullspot_set_token)
...
// Cleared on: PlayerEvent::Playing (success), nullspot_stop/disconnect/shutdown/cleanup (canceled).
...
#[no_mangle]
pub extern "C" fn nullspot_free_string(s: *mut c_char) {
    if !s.is_null() {
        unsafe {
...
#[no_mangle]
pub extern "C" fn nullspot_register_queue_callback(callback: extern "C" fn(*const c_char)) {
...
#[no_mangle]
pub extern "C" fn nullspot_register_playback_state_callback(
...
#[no_mangle]
pub extern "C" fn nullspot_register_state_update_callback(callback: extern "C" fn()) {
...
#[no_mangle]
pub extern "C" fn nullspot_register_volume_callback(callback: extern "C" fn(u16)) {
...
#[no_mangle]
pub extern "C" fn nullspot_register_loading_callback(callback: extern "C" fn(*const c_char)) {
...
#[no_mangle]
pub extern "C" fn nullspot_register_queue_changed_callback(callback: extern "C" fn(*const c_char)) {
...
#[no_mangle]
pub extern "C" fn nullspot_register_session_disconnected_callback(callback: extern "C" fn()) {
...
#[no_mangle]
pub extern "C" fn nullspot_register_session_connected_callback(callback: extern "C" fn()) {
...
#[unsafe(no_mangle)]
pub extern "C" fn nullspot_register_session_client_changed_callback(
    callback: extern "C" fn(*const c_char),
) {
...
#[no_mangle]
pub extern "C" fn nullspot_register_set_queue_callback(callback: extern "C" fn(*const c_char)) {
...
#[no_mangle]
pub extern "C" fn nullspot_register_active_device_callback(callback: extern "C" fn(*const c_char)) {
...
#[no_mangle]
pub extern "C" fn nullspot_register_token_request_callback(callback: extern "C" fn()) {
...
#[no_mangle]
pub extern "C" fn nullspot_set_token(token: *const c_char) {
    if token.is_null() {
        debug!("nullspot_set_token: token is null");
        return;
    }
    ...
    let token_str = unsafe {
        match CStr::from_ptr(token).to_str() {
            Ok(s) => s.to_string(),
            Err(_) => {
                debug!("nullspot_set_token: invalid token string");
                return;
            }
        }
    };
    ...
    debug!(
        "nullspot_set_token: received token ({} chars)",
        token_str.len()
    );
    ...
    if let Some(sender) = pending.take() {
        if sender.send(token_str).is_err() {
            debug!("nullspot_set_token: receiver dropped");
        }
    } else {
        debug!("nullspot_set_token: no pending token request");
    }
}
...
#[no_mangle]
pub extern "C" fn nullspot_register_connection_state_callback(
    callback: extern "C" fn(*const c_char),
) {
...
#[no_mangle]
pub extern "C" fn nullspot_register_audio_data_callback(
...
#[no_mangle]
pub extern "C" fn nullspot_register_audio_control_callback(callback: extern "C" fn(u8)) {
...
#[no_mangle]
pub extern "C" fn nullspot_get_connection_state() -> *mut c_char {
...
fn build_connection_state_info() -> ConnectionStateInfo {
    ...
    ConnectionStateInfo {
        ...
        device_id,
        device_name: "NullSpot".to_string(),
...
///
/// This does NOT touch the global SESSION used for Spotify Connect — it spins up
/// a separate Session, connects, calls login5().auth_token(), and drops it. The
/// resulting token inherits the desktop client's privileges and bypasses Dev
/// Mode restrictions.
///
/// Caller must free the returned string using nullspot_free_string().
/// Returns "ERR: <reason>" string on failure.
#[no_mangle]
pub extern "C" fn nullspot_mint_login5_token_from_oauth(
    access_token: *const c_char,
) -> *mut c_char {
    ensure_logger();
    let token_str = if access_token.is_null() {
        return CString::new("ERR: null token").unwrap().into_raw();
    } else {
        match unsafe { CStr::from_ptr(access_token) }.to_str() {
            Ok(s) => s.to_string(),
            Err(_) => return CString::new("ERR: invalid utf8").unwrap().into_raw(),
        }
    };
    ...
    let payload = RUNTIME.block_on(async move {
        ...
        let credentials =
            librespot_core::authentication::Credentials::with_access_token(&token_str);
        let cache = match Cache::new(None::<std::path::PathBuf>, None, None, None) {
            Ok(c) => c,
            Err(e) => return format!("ERR: cache: {}", e),
        };
        let session = Session::new(session_config, Some(cache));

        let connect_result =
            tokio::time::timeout(Duration::from_secs(15), session.connect(credentials, false))
                .await;
        match connect_result {
            Ok(Ok(())) => {}
            Ok(Err(e)) => return format!("ERR: connect: {:?}", e),
            Err(_) => return "ERR: connect timeout".to_string(),
        }

        match tokio::time::timeout(Duration::from_secs(10), session.login5().auth_token()).await {
            Ok(Ok(token)) => token.access_token,
            Ok(Err(e)) => format!("ERR: login5: {:?}", e),
            Err(_) => "ERR: login5 timeout".to_string(),
        }
    });
...
#[no_mangle]
pub extern "C" fn nullspot_get_login5_token() -> *mut c_char {
    ...
    let result = RUNTIME.block_on(async {
        tokio::time::timeout(Duration::from_secs(5), session.login5().auth_token()).await
    });
...
/// Returns the on-disk audio cache directory used by librespot.
/// macOS convention: ~/Library/Caches/NullSpot/audio.
/// Returns None when $HOME isn't set, in which case caching is disabled.
...
/// 1 GiB cap on the audio cache. librespot uses an LRU-style limiter that prunes
/// the oldest-touched files when the total exceeds this size.
...
fn create_session(
    ...
    // Enable disk cache for audio files so subsequent plays of the same track
    // skip the CDN download entirely. Fall back to no cache if the directory
    // can't be created (e.g. unusual sandbox state) — better to keep playing
    // than to refuse to start.
    let audio_path = audio_cache_path();
    let cache = match Cache::new(
        ...
        Ok(c) => {
            if let Some(p) = &audio_path {
                debug!("Audio cache enabled at {:?} (1 GiB limit)", p);
            } else {
                debug!("Audio cache disabled: $HOME not set");
            }
            c
        }
        Err(e) => {
            debug!("Audio cache disabled (init failed: {}). Falling back to no cache.", e);
            Cache::new(
                None::<std::path::PathBuf>,
                None,
                None::<std::path::PathBuf>,
                                    ...
                                    let result = match &p.request {
                                        PendingPlayRequest::Uri(uri, track_index) => {
                                            ...
                                            match std::ffi::CString::new(uri.as_str()) {
                                                Ok(cstr) => nullspot_play_uri(cstr.as_ptr(), *track_index),
                                                Err(_) => return,
                                            }
                                        }
                                        PendingPlayRequest::Tracks(json) => {
                                            ...
                                            match std::ffi::CString::new(json.as_str()) {
                                                Ok(cstr) => nullspot_play_tracks(cstr.as_ptr()),
                                                Err(_) => return,
                                            }
                                        }
...
#[no_mangle]
pub extern "C" fn nullspot_force_reconnect() -> i32 {
    ...
    debug!("[WAKE +0ms] nullspot_force_reconnect called at {}", wake_ts);
...
async fn soft_reconnect_async(
    ...
    let device_id = {
        ...
        guard
            .clone()
            .unwrap_or_else(|| format!("nullspot_{}", std::process::id()))
    };
    ...
    if reactivate_after_reconnect {
        ...
        debug!("Soft reconnect: keeping NullSpot inactive (was_active=false)");
    }
...
#[no_mangle]
pub extern "C" fn nullspot_init_player(access_token: *const c_char) -> i32 {
    ensure_logger();
...
async fn init_player_async(access_token: &str, activate_after_connect: bool) -> Result<(), String> {
    ...
    let device_id = format!("nullspot_{}", std::process::id());
    {
        let mut device_id_guard = DEVICE_ID.lock().unwrap();
...
/// Error codes:
/// -1 = general error
/// -2 = channel closed, needs reinit (call nullspot_init_player again)
...
#[no_mangle]
pub extern "C" fn nullspot_is_session_connected() -> i32 {
...
/// This is used when NullSpot is the active device - state changes happen locally
...
#[no_mangle]
pub extern "C" fn nullspot_play_tracks(track_uris_json: *const c_char) -> i32 {
    debug!("nullspot_play_tracks called");
...
/// Returns 0 on success, -1 on error.
#[no_mangle]
pub extern "C" fn nullspot_play_uri(uri_or_url: *const c_char, track_index: i32) -> i32 {
    ...
    debug!(
        "nullspot_play_uri called: uri={}, track_index={}",
        uri_str, track_index
    );
    ...
    match spirc_guard.as_ref() {
        Some(spirc) => {
            ...
            let load_request = if uri_str.starts_with("spotify:track:") {
                // Legacy single-track behavior - prefer using nullspot_play_radio instead
...
#[no_mangle]
pub extern "C" fn nullspot_pause() -> i32 {
    debug!("nullspot_pause called");
    if let Err(e) = require_session_connected() {
        return e;
    }
...
/// Note: nullspot_disconnect() already handles this internally.
#[no_mangle]
pub extern "C" fn nullspot_clear_audio_buffer() {
    debug!("nullspot_clear_audio_buffer called");
...
/// Returns 0 on success, -1 on error, -2 if channel closed (needs reinit).
#[no_mangle]
pub extern "C" fn nullspot_resume() -> i32 {
    debug!("nullspot_resume called");
    if let Err(e) = require_session_connected() {
        return e;
    }
...
/// Returns 0 on success, -1 on error.
#[no_mangle]
pub extern "C" fn nullspot_stop() -> i32 {
    debug!("nullspot_stop called");
...
/// Returns 0 on success, -1 on error.
#[no_mangle]
pub extern "C" fn nullspot_shutdown() -> i32 {
    debug!("nullspot_shutdown called");
...
/// Returns 0 on success, -1 on error.
#[no_mangle]
pub extern "C" fn nullspot_disconnect() -> i32 {
    debug!("nullspot_disconnect called - disconnecting for sleep");
    ...
    if let Some(spirc) = spirc_guard.as_ref() {
        ...
        let _ = spirc.pause();
        debug!("nullspot_disconnect: paused playback");
        ...
        proxy_sink::ProxySink::clear_buffer();
        debug!("nullspot_disconnect: audio buffer cleared");

        // Now shutdown Spirc (disconnect from Spotify Connect)
        let spirc_guard = SPIRC.lock().unwrap();
        if let Some(spirc) = spirc_guard.as_ref() {
            if spirc.shutdown().is_ok() {
                debug!("nullspot_disconnect: spirc shutdown complete");
                return 0;
            }
        }
    }
...
#[no_mangle]
pub extern "C" fn nullspot_cleanup() {
    debug!("nullspot_cleanup called - clearing all state");
    ...
    // Shutdown Spirc first - this terminates the spirc_task and closes the dealer
    shutdown_spirc("nullspot_cleanup");
    ...
    debug!("nullspot_cleanup complete - ready for reinitialization");
}

/// Returns 1 if currently playing, 0 otherwise.
#[no_mangle]
pub extern "C" fn nullspot_is_playing() -> i32 {
    if IS_PLAYING.load(Ordering::SeqCst) {
        1
    } else {
        0
...
#[no_mangle]
pub extern "C" fn nullspot_is_active_device() -> i32 {
    if IS_ACTIVE_DEVICE.load(Ordering::SeqCst) {
        1
    } else {
        0
...
#[no_mangle]
pub extern "C" fn nullspot_get_position_ms() -> u32 {
...
/// Returns 0 on success, -1 on error, -2 if channel closed (needs reinit).
#[no_mangle]
pub extern "C" fn nullspot_next() -> i32 {
    debug!("nullspot_next called");
    if let Err(e) = require_session_connected() {
        return e;
    }
    let spirc_guard = SPIRC.lock().unwrap();
...
/// Returns 0 on success, -1 on error, -2 if channel closed (needs reinit).
#[no_mangle]
pub extern "C" fn nullspot_previous() -> i32 {
    debug!("nullspot_previous called");
    if let Err(e) = require_session_connected() {
        return e;
    }
    let spirc_guard = SPIRC.lock().unwrap();
...
/// Returns 0 on success, -1 on error, -2 if channel closed (needs reinit).
#[no_mangle]
pub extern "C" fn nullspot_seek(position_ms: u32) -> i32 {
    debug!("nullspot_seek called: {}ms", position_ms);
    if let Err(e) = require_session_connected() {
        return e;
    }
    let spirc_guard = SPIRC.lock().unwrap();
...
/// Returns 0 on success, -1 on error.
#[no_mangle]
pub extern "C" fn nullspot_play_radio(track_uri: *const c_char) -> i32 {
    ...
    debug!("nullspot_play_radio called: {}", uri_str);
...
/// Returns 0 on success, -1 on error, -2 if channel closed (needs reinit).
#[no_mangle]
pub extern "C" fn nullspot_set_volume(volume: u16) -> i32 {
    debug!("nullspot_set_volume called: {}", volume);
    if let Err(e) = require_session_connected() {
        return e;
    }
    let spirc_guard = SPIRC.lock().unwrap();
...
/// Returns 0 on success, -1 on error, -2 if channel closed (needs reinit).
#[no_mangle]
pub extern "C" fn nullspot_set_shuffle(enabled: bool) -> i32 {
    debug!("nullspot_set_shuffle called: {}", enabled);
    if let Err(e) = require_session_connected() {
        return e;
    }
    let spirc_guard = SPIRC.lock().unwrap();
...
#[no_mangle]
pub extern "C" fn nullspot_set_bitrate(bitrate: u8) {
...
#[no_mangle]
pub extern "C" fn nullspot_get_bitrate() -> u8 {
...
#[no_mangle]
pub extern "C" fn nullspot_set_gapless(enabled: bool) {
...
#[no_mangle]
pub extern "C" fn nullspot_get_gapless() -> bool {
...
#[no_mangle]
pub extern "C" fn nullspot_set_initial_volume(volume: u16) {
...
/// Returns 0 on success, -1 on error.
#[no_mangle]
pub extern "C" fn nullspot_transfer_to_local() -> i32 {
    debug!("nullspot_transfer_to_local called");
    if let Err(e) = require_session_connected() {
        return e;
    }
    let spirc_guard = SPIRC.lock().unwrap();
...
#[no_mangle]
pub extern "C" fn nullspot_transfer_playback(to_device_id: *const c_char) -> i32 {
    ...
    debug!("nullspot_transfer_playback called: {}", to_device_str);
...
#[no_mangle]
pub extern "C" fn nullspot_is_spirc_ready() -> i32 {
...
/// Returns 0 on success, -1 on error.
#[no_mangle]
pub extern "C" fn nullspot_add_to_queue(uri: *const c_char) -> i32 {
    if uri.is_null() {
    ...
    debug!("[NullSpot] nullspot_add_to_queue called: {}", uri_str);
    ...
    match spirc_guard.as_ref() {
        Some(spirc) => match spirc.add_to_queue(spotify_uri) {
            Ok(_) => {
                debug!("[NullSpot] add_to_queue succeeded");
                0
            }
            Err(e) => {
                debug!("Add to queue error: {:?}", e);
