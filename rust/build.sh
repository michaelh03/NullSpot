...
# Build script for the NullSpot Rust library
...
if [ "$CONFIGURATION" = "Debug" ]; then
    CARGO_FLAGS=""
    BUILD_TYPE="debug"
    echo "Building NullSpot Rust library (DEBUG) for platform: $PLATFORM_NAME"
else
    CARGO_FLAGS="--release"
    BUILD_TYPE="release"
    echo "Building NullSpot Rust library (RELEASE) for platform: $PLATFORM_NAME"
fi
...
case "$PLATFORM_NAME" in
    macosx*)
        ...
        cp "$RUST_DIR/target/aarch64-apple-darwin/$BUILD_TYPE/libnullspot_rust.a" "$OUTPUT_DIR/lib/"
        ;;
    iphoneos*)
        echo "Building for iOS device (aarch64)..."
        cargo build $CARGO_FLAGS --target aarch64-apple-ios
        cp "$RUST_DIR/target/aarch64-apple-ios/$BUILD_TYPE/libnullspot_rust.a" "$OUTPUT_DIR/lib/"
        ;;
    iphonesimulator*)
        echo "Building for iOS Simulator (aarch64)..."
        cargo build $CARGO_FLAGS --target aarch64-apple-ios-sim
        cp "$RUST_DIR/target/aarch64-apple-ios-sim/$BUILD_TYPE/libnullspot_rust.a" "$OUTPUT_DIR/lib/"
        ;;
    *)
...
cp "$RUST_DIR/include/nullspot_rust.h" "$OUTPUT_DIR/include/"
cp "$RUST_DIR/include/module.modulemap" "$OUTPUT_DIR/include/"

echo "Build complete!"
echo "Static library: $OUTPUT_DIR/lib/libnullspot_rust.a"
echo "Header: $OUTPUT_DIR/include/nullspot_rust.h"
