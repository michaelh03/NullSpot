...
This document covers building NullSpot from source and contributing to the project.

## Architecture

NullSpot is a Swift/SwiftUI app that integrates with Spotify using the [librespot](https://github.com/librespot-org/librespot) Rust library for OAuth authentication.
...
This compiles the Rust code into a static library at `build/rust/lib/libnullspot_rust.a`.

### 2. Build the Swift app

Open `NullSpot.xcodeproj` in Xcode and build (⌘B), or:
...
```
NullSpot/
├── NullSpot/                    # Swift source files
│   ├── NullSpotApp.swift        # App entry point
...
│   ├── include/nullspot_rust.h  # C header for FFI
│   └── build.sh                 # Build script
├── build/rust/                  # Built Rust artifacts
│   ├── lib/libnullspot_rust.a   # Static library
│   └── include/                 # Headers + module map
└── NullSpot.xcodeproj           # Xcode project
```
